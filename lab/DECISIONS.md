...
## Run 70 — half50-w2400: elite winner-only policy at half budget (2026-08-09)

Learner-designed experiment: same as fullbudget-69-untied except the corpus (decisive winner≥2400 + draws max≥2400, nobullet — 0.645% acceptance, 1.74M games / 146.5M positions, 84.3 pos/game, 13.4% draws, median winner 2490) and a new `policy_winner_only` mask (policy CE on mover-won rows only, 41.2% of corpus; zero-gradient on other rows proven by a scrambled-target bit-identity test). Value trains on every ply, mover-relative — the learner's "reverse sides and invert value" comes free from flip canonicalization. Draws kept per the 67b draw-blind-judge lesson.

**Continuable-checkpoint design:** main leg on the FULL 1e18-horizon cosine (`schedule_total_steps` 189,956) stopped at 94,978 steps; then a branched anneal (`schedule_from_step` window, 2,850 steps from the exact stop-point LR 6.4983e-4 → 0). Standalone lineage **0.49941e18**; the unannealed step-94,950 resume checkpoint (optimizer intact, mid-schedule) is the future-continuation seed.

**Results (April-nobullet ruler):** main leg 1.5444 / 50.38 / 59.55; **annealed 1.5212 / 51.57 / 59.56** (anneal bought −0.023 loss / +1.2 top-1 for 1.5% budget). Continuity ruler: 1.6318 / 49.87 / 58.31 vs fullbudget-68's 1.4060 / 54.37 / 59.12 — the elite specialization costs ~4.5 top-1 on the broad-population ruler with an all-TC penalty of +0.111 (vs 68's +0.089). Imitation rulers can't settle strength (67b); the arena will.

**Published: `peterwooden/chess-gpt-sore-winner-thinking` @ `1f32995b6530312657be6ac52bae6cfa57b757ea`** — v3-thinking adapter (byte-identical entry.js to loose-pieces), untied ONNX parity 100% argmax, probes 6/6, no clock overshoot at 10s or 1s, runrecord with full lineage split. Siblings for comparison: fullbudget-68 (tied, 0.97e18), loose-pieces (untied, 0.97e18), sore-winner (untied, elite winner-only, 0.4994e18 + warm checkpoint).

## Run 70 completion — sore-winner continued to full budget (2026-08-09)

The half-budget gauntlet triggered the pre-agreed continue rule emphatically: **sore-winner (0.4994e18) beat loose-pieces 3.5–0.5** (its own full-budget broad-diet untied twin, matched clocks 1s/3s) **and swept minichess-gpt-v1-final 2–0 at 1s** — 5.5/6 overall, the sharpest ruler-inversion yet (it trails ~4.5 top-1 on imitation rulers). The elitism-hurts prior from 67/67b does not extend to policy-target masking: filtering the *targets* (winner≥2400 moves) while the value head keeps full coverage is a different — and evidently much better — animal than filtering games.

Continuation: resumed results3/half50-w2400.resume.pt (step 94,950, mid-cosine) → 189,950 steps, epochs=2 for the data wrap (no discontinuity at the boundary), natural anneal completion. **Final (April-nobullet): 1.5000 / 52.52 / 59.82.** Full lineage ≈ 0.9699e18 (runrecord carries the parent split). **Published as the new Latest of `peterwooden/chess-gpt-sore-winner-thinking` @ `45e1befcac7decc7dab45a08dce071c0a3360065`**; the half-budget artifact stays pinned at 1f32995b. Validator/probes/smokes green; continuity eval trailing. Next instrument: full-clock arena games vs v1-final.
